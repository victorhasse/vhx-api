export function createRequestError(message, statusCode = 400) {
...
export function convertPriceToCents(value, productName) {
  ...
  if (!Number.isFinite(price) || price < 0) {
    throw createRequestError(`Preço inválido para o produto "${productName}"`);
  }
...
export function normalizeRequestedItems(items) {
  ...
  return items.map((item) => {
    ...
    const rawVariantId = item.variantId ?? item.product_variant_id ?? null;
    const variantId =
      rawVariantId === null || rawVariantId === ""
        ? null
        : Number(rawVariantId);
    ...
    if (
      ...
      throw createRequestError("O carrinho possui um item inválido");
    }
