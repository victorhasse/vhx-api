...
describe("createRequestError", () => {
  it("cria um erro de requisição", () => {
    const error = createRequestError("Dados inválidos", 422);
...
describe("convertPriceToCents", () => {
  it("converte número para centavos", () => {
    expect(convertPriceToCents(349.99, "Camiseta")).toBe(34999);
  });

  it("converte preço com vírgula", () => {
    expect(convertPriceToCents("79,90", "Boné")).toBe(7990);
  });

  it("rejeita um preço inválido", () => {
    expect(() => convertPriceToCents("preço inválido", "Produto")).toThrow(
...
describe("normalizeRequestedItems", () => {
  ...
  it("aceita productId como identificador do produto", () => {
    expect(
      normalizeRequestedItems([
        {
          productId: 10,
  ...
  it("utiliza tamanho nulo quando ausente", () => {
    const [item] = normalizeRequestedItems([
      {
        id: 5,
        quantity: 1,
      },
  ...
  it("rejeita carrinho vazio", () => {
    expect(() => normalizeRequestedItems([])).toThrow("Carrinho vazio");
  });
  ...
  it("rejeita quantidade inválida", () => {
    ...
    ).toThrow("O carrinho possui um item inválido");
  });
});

describe("groupQuantitiesByProduct", () => {
  it("soma itens repetidos do mesmo produto", () => {
    const quantities = groupQuantitiesByProduct([
      {
        productId: 10,
        quantity: 2,
      },
...
describe("checkout com variantes", () => {
  ...
  it("agrupa quantidades da mesma variante", () => {
    const quantities = groupQuantitiesByVariant([
      {
        productId: 10,
        variantId: 25,
        quantity: 2,
