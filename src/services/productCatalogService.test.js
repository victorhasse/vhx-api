...
describe('normalizeSlug', () => {
  it('normaliza espaços, acentos e maiúsculas', () => {
    expect(normalizeSlug('  Verde Limão  '))
      .toBe('verde-limao')
  })

  it('remove caracteres especiais', () => {
    expect(normalizeSlug('Azul & Branco!'))
      .toBe('azul-branco')
  })
...
describe('isValidImageUrl', () => {
  it('aceita uma URL HTTPS', () => {
    expect(
      isValidImageUrl(
        'https://example.com/produto.webp'
      )
    ).toBe(true)
  })
  ...
  it('aceita um caminho relativo iniciado por barra', () => {
    expect(
      isValidImageUrl('/images/produto.webp')
    ).toBe(true)
  })
...
describe('normalizeSku', () => {
  it('normaliza espaços e letras minúsculas', () => {
    expect(normalizeSku(' camiseta preta m '))
      .toBe('CAMISETA-PRETA-M')
  })

  it('mantém um SKU que já está normalizado', () => {
    expect(normalizeSku('VHX-001-G'))
      .toBe('VHX-001-G')
  })
})
...
describe('normalizeOptionalPrice', () => {
  ...
  it('aceita preço com vírgula', () => {
    expect(normalizeOptionalPrice('129,90'))
      .toBe(129.9)
  })

  it('arredonda o preço para duas casas decimais', () => {
    expect(normalizeOptionalPrice(19.999))
      .toBe(20)
  })

  it('rejeita preço negativo', () => {
    expect(normalizeOptionalPrice(-10))
      .toBeUndefined()
  })

  it('rejeita preço não numérico', () => {
    expect(normalizeOptionalPrice('inválido'))
      .toBeUndefined()
  })
})