...
async function cancelOrderAndRestoreStock(paymentIntent) {
  await sequelize.transaction(async (transaction) => {
    ...
    for (const item of orderItems) {
      if (item.product_variant_id) {
        const currentQuantity =
          quantitiesByVariant.get(item.product_variant_id) || 0;
       ...
       */
      const currentQuantity = quantitiesByProduct.get(item.product_id) || 0;

      quantitiesByProduct.set(item.product_id, currentQuantity + item.quantity);
    }
    ...
    console.log(`↩️ Estoque restaurado para o pedido ${order.id}`);
  });
}
...
export async function stripeWebhook(req, res) {
  ...
  try {
    ...
    console.error("Assinatura inválida do webhook:", error.message);
  ...
  try {
    switch (event.type) {
      case "payment_intent.succeeded":
        await confirmPaidOrder(event.data.object);

        console.log(
          `✅ Pagamento ${event.data.object.id} processado pelo webhook`,
        );
        break;

      case "payment_intent.canceled":
        await cancelOrderAndRestoreStock(event.data.object);
        break;
